...
.DESCRIPTION
    ...
    A few notes carry photos from scripts/seed-assets/, uploaded through the real media endpoint, so
    a seeded instance exercises image attachments as well as text. Needs PowerShell 7+ (the
    multipart upload uses Invoke-RestMethod -Form).
...
# Uploads one image to a note's media endpoint. Multipart rather than JSON, so it goes through
# Invoke-RestMethod -Form (PowerShell 7+) instead of Invoke-Api.
function Invoke-ApiUpload {
    ...
    return Invoke-RestMethod -Method POST -Uri "$BaseUrl$Path" -Headers $headers -Form @{
...
$textNotes = @(
    @{ t = "Welcome to keepIT";        c = "indigo"; lists = @();             b = "Notes, checklists and photos, synced live across every device you sign in on. This sandbox was filled by scripts/seed-dev-data.ps1 — re-run it with -Reset for a clean slate." }
...
# Notes carrying photos. The note is created first and each image is then uploaded to its media
# endpoint — the same two steps every real client takes. A missing asset is warned about and skipped
# so the rest of the seed still lands.
$imageNotes = @(
    @{ t = "Puglia coast road"; c = "teal";  lists = @("Travel");   images = @("coastal-town.jpg", "aperitivo-terrace.jpg")
       b = "Vieste — the old town drops straight into the water. Best light an hour before sunset, then a spritz on the terrace on the way back up." }
    @{ t = "Museum day";        c = "mauve"; lists = @("Personal"); images = @("museum-sculpture.jpg")
       b = "Room 12, late afternoon, light coming in sideways. Everyone walks straight past this one." }
    @{ t = "Garden visitors";   c = "sage";  lists = @("Personal"); images = @("hummingbird.jpg")
       b = "Same branch every morning around eight. The feeder needs refilling twice a week now." }
)

$assetDir = Join-Path $PSScriptRoot "seed-assets"
$imageCount = 0

foreach ($n in $imageNotes) {
    $listIds = @($n.lists | ForEach-Object { $lists[$_] })
    $note = Invoke-Api -Method POST -Path "/api/notes" -Body @{
        type    = "Text"
        title   = $n.t
    ...
    foreach ($image in $n.images) {
        $file = Join-Path $assetDir $image
        if (-not (Test-Path -LiteralPath $file)) {
            Write-Warning "missing seed image, skipped: $file"
            continue
        }
        Invoke-ApiUpload -Path "/api/notes/$($note.id)/media" -FilePath $file | Out-Null
        $imageCount++
    }
}

Write-Host "  created $($createdNotes.Count) notes, attached $imageCount image(s)" -ForegroundColor Green
...
Write-Host "  Lists: $($lists.Count)   Notes: $($createdNotes.Count) (some pinned / archived / trashed)   Images: $imageCount"

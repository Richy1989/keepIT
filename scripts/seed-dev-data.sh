...
#
# A few notes carry photos from scripts/seed-assets/, uploaded through the real media endpoint, so a
# seeded instance exercises image attachments as well as text.
#
# Usage:
...
note_count=0
image_count=0
ASSET_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)/seed-assets"
...
# upload_image NOTE_ID FILE — posts one image to the note's media endpoint. Multipart rather than
# JSON, so it bypasses request() and calls curl directly.
upload_image() {
    local note_id="$1" file="$2"
    local args=(-sS -o /dev/null -X POST "${BASE_URL}/api/notes/${note_id}/media" -F "file=@${file}" -w '%{http_code}')
    [[ -n "$TOKEN" ]] && args+=(-H "Authorization: Bearer ${TOKEN}")
    local code
    code="$(curl "${args[@]}" 2>/dev/null)" || code="000"
    [[ "$code" == "201" ]] || { echo "  upload failed (HTTP $code): $(basename "$file")" >&2; return 1; }
}

# add_image_note TITLE COLOR LISTS_CSV BODY IMAGE... — a text note plus its photo attachments. The
# note is created first and each image is then uploaded, the same two steps every real client takes.
# A missing asset is warned about and skipped so the rest of the seed still lands.
add_image_note() {
    local title="$1" color="$2" lists="$3" body="$4"; shift 4
    add_text "$title" "$color" "$lists" "$body"
    local id="${NOTE_ID[$title]}" image file
    for image in "$@"; do
        file="${ASSET_DIR}/${image}"
        if [[ ! -f "$file" ]]; then
            echo "  missing seed image, skipped: $file" >&2
            continue
        fi
        upload_image "$id" "$file" && image_count=$((image_count + 1))
    done
}

add_text "Welcome to keepIT"        "indigo"  ""             "Notes, checklists and photos, synced live across every device you sign in on. This sandbox was filled by scripts/seed-dev-data.sh — re-run it with --reset for a clean slate."
...
add_image_note "Puglia coast road" "teal"  "Travel"   "Vieste — the old town drops straight into the water. Best light an hour before sunset, then a spritz on the terrace on the way back up." "coastal-town.jpg" "aperitivo-terrace.jpg"
add_image_note "Museum day"        "mauve" "Personal" "Room 12, late afternoon, light coming in sideways. Everyone walks straight past this one." "museum-sculpture.jpg"
add_image_note "Garden visitors"   "sage"  "Personal" "Same branch every morning around eight. The feeder needs refilling twice a week now." "hummingbird.jpg"

echo "${C_GREEN}  created ${note_count} notes, attached ${image_count} image(s)${C_RESET}"
...
echo "  Lists: ${#LIST[@]}   Notes: ${note_count} (some pinned / archived / trashed)   Images: ${image_count}"

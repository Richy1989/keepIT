...
- **API tests** live in `keepIT/keepITCore.Tests/` (xUnit): the real API in-process via `WebApplicationFactory`, each host on a throwaway SQLite data root. They cover the SQLite schema reconciler (an older database comes up to date without losing data), note media end to end, where password-reset links point, and that SMTP never falls back to plain text. `KeepItApiFactory` takes per-host `Settings` and `Services` overrides (e.g. `CapturingEmailSender` to read outgoing mail); `FakeSmtpServer` is a loopback SMTP server that records what it receives. Parallelization is off on purpose — `FolderManagement.RootPath` is process-wide static, so two hosts at once would write each other's media. When you change an entity or a media rule, extend these.
